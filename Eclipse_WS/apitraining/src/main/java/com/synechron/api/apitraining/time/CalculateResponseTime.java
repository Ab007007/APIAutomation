package com.synechron.api.apitraining.time;
...
//	@Test
	public void verifyTime()
	{
		...
		long time = given().spec(rSpec).
		when().
			get("/1/boards/SIz5gELw").time();
		System.out.println("******************" + time);
		
	}

	@Test
	public void verifyTimeInSec()
	{
		
		
		long time = given().spec(rSpec).
		when().
			get("/1/boards/SIz5gELw").timeIn(TimeUnit.SECONDS);
		System.out.println("******************" + time);
		
	}
	
	@Test
	public void verifyTimeWithValidation()
	{
		...
		then(). 
			time(Matchers.lessThan(500L), TimeUnit.MILLISECONDS);
	...
	public void proxy()
	{
		...
		given().spec(rSpec).proxy("localhost", 5555).
		when().
			get("/1/boards/SIz5gELw").
		then(). 
			time(Matchers.lessThan(500L), TimeUnit.MILLISECONDS);
